...
\employer{Facebook}{Menlo Park, CA (remote)}{Software Engineering Intern}{June 2020 --- August 2020}
	\begin{achievements}
	\item Implemented UI and backend of core features on Facebook website in \texttt{React.js} and \texttt{PHP}
	\item Refactored backend infrastructure to support flexible component rendering and faster \texttt{GraphQL} data fetching
	\end{achievements}
...
\vspace{2pt}
\begin{achievements}
\item Lead a team of four to develop an interactive web app on Heroku using a Flask framework, web-scraping using \texttt{Beautiful Soup\texttrademark} and \texttt{Google Firebase\texttrademark}
\end{achievements}
...
\vspace{2pt}
\begin{achievements}
\item Implemented a programs to detect port scanning and ARP spoofing on a network and conduct man-in-the-middle DNS attacks.
\item Required to self-teach \texttt{Golang}
\end{achievements}
...
\vspace{2pt}
\begin{achievements}
\item Implemented pairing heap data structure based on research papers and applied to priority queue
\end{achievements}
...
\header{Additional}
\begin{achievements}
\item Relevant languages: C++, Python, Javascript (React.js), PHP
\item Technologies: Visual Studio Code, \LaTeX, Git, Mercurial
